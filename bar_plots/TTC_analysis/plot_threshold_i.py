...
def get_ttc_results(model_name):
    ...
    RESULT_PATHS = {
        "ttc": {
            "clean": {
                "base_path": (
                    "../../Final_Results_corrected_ca_tau_Counter_Attack_I/"
                    "{model}/{dataset}/"
...
if __name__ == "__main__":
    ...
    output_dir = "I_ttc_without_threshold"
    ...
    output_root = os.path.join("I_ttc_with_our_threshold", model_name)
    os.makedirs(output_root, exist_ok=True)

    # --- Opposite Gating Logic (TTC if <= threshold) ---
    output_root_default = os.path.join("I_ttc_with_reported_threshold", model_name)
    ...
    for noise_type in ["Gaussian", "Uniform"]:
        for noise_param in next(
            ...
            for current_output_root, current_logic in [
                ...
                bars1 = plt.bar(x - width / 2, clean_accs, width, label="Clean", color='#4682B4', edgecolor='black',
