...
def get_zs_results(model_name):
    ...
    RESULT_PATHS = {
        "zero_shot": {
            "clean": {
                "base_path": (
                    "../../Final_Results_corrected_ca_tau_I/"
                    "{model}/{dataset}/"
    ...
    return TRUE_LABELS_DATASET, ZS_CLEAN_PREDS_DATASET, ZS_ADV_PREDS_DATASET, 0
...
# Treat all epsilon=0 attacks as "Clean". Different pipelines may name the clean attack
# either eps_0.0_steps_0 or eps_0.0_steps_100 (or image_only variants).
CLEAN_ATTACK_KEYS = {
    "eps_0.0_steps_0",
    "eps_0.0_steps_100",
}
...
def normalize_attack_key(attack: str) -> str:
    """Normalize attack folder keys so clean runs are treated consistently."""
    if attack in CLEAN_ATTACK_KEYS:
        # Canonicalize all clean runs to one key to simplify downstream logic.
        return "eps_0.0_steps_100"

    # Some folders may include suffixes like *_image_only_attack_prm.
    if attack.startswith("eps_0.0"):
        return "eps_0.0_steps_100"
...
def parse_experiment_folder_name(folder_name: str) -> Optional[Dict[str, Any]]:
    ...
    # noise type: token after Added_Noise_ (allow underscores/digits)
    m_noise = re.search(r"_Added_Noise_([^_]+)_", folder_name)
...
def get_aggregated_results(root: str, selected_attacks: Optional[List[str]] = None) -> dict:
    ...
    for dataset in dataset_names:
        ...
        for exp_name in sorted(os.listdir(dataset_dir)):
            ...
            # Mapping: image_only variants -> their base keys
            if attack == "eps_0.0_steps_0_image_only_attack_prm":
                attack = "eps_0.0_steps_0"
            if attack == "eps_0.0_steps_100_image_only_attack_prm":
                attack = "eps_0.0_steps_100"

            # Normalize all clean variants to one canonical key.
            attack = normalize_attack_key(attack)
...
def main():
    ...
    parser.add_argument(
        "--root_diff_ratio",
        type=str,
        default="../../Diffratio_v3_I/vit_l_14_datacomp_1b",
    ...
    parser.add_argument(
        "--out_results",
        type=str,
        default="aggregated_results_v2_I.json",
        help="Output JSON filename (written under --root unless absolute path)",
    )
    ...
    # We canonicalize all eps_0.* variants to eps_0.0_steps_100 (see normalize_attack_key).
    selected_attacks = ["eps_0.0_steps_100", "eps_4.0_steps_100"]
    ...
    for dataset in datasets:
        ...
        for attack in selected_attacks:
            ...
            if normalize_attack_key(attack) == "eps_0.0_steps_100":
                            ...
                            for noise_param in dr_noise_params:
                                ...
                                for tau_type in dr_tau_types:
                                    ...
                                    if len(ca_preds) != len(diff_ratios) or len(ca_preds) != len(zs_preds):
                                        print(
                                            f"          Warning: Length mismatch. ca_preds: {len(ca_preds)}, diff_ratios: {len(diff_ratios)}, zs_preds: {len(zs_preds)}. Skipping.")
                                        continue
    ...
    output_base_dir = "plots_threshold_bar_I"
    ...
    for attack in final_results_average_dic:
        for dr_label in final_results_average_dic[attack]:
            for ca_label in final_results_average_dic[attack][dr_label]:
                ...
                attack_keys = ["eps_0.0_steps_100", "eps_4.0_steps_100"]
                if all(ak in final_results_average_dic and dr_label in final_results_average_dic[ak] and ca_label in
                       final_results_average_dic[ak][dr_label] for ak in attack_keys):
    ...
    for dr_label, ca_label in combinations:
        ...
        if len(attack_accs) == len(attack_keys):
            ...
            for max_idx in max_indices:
                plt.plot(x[max_idx], net_accs[max_idx], marker='*', color='gold', markersize=25,
            ...
            for attack_key, accs in attack_accs.items():
                ...
                for max_idx in max_indices:
                    plt.plot(x[max_idx], accs[max_idx], marker='*', color='gold', markersize=15,
                             markeredgecolor='black', zorder=5)
        ...
        for idx, dr_label in enumerate(dr_labels):
            ...
            if len(attack_accs) == len(attack_keys):
                ...
                for max_idx in [i for i, v in enumerate(net_accs) if v == max_net_acc]:
                    ax.plot(x[max_idx], net_accs[max_idx], marker='*', color='gold', markersize=25,
                            markeredgecolor='black', zorder=5)
                    ax.annotate(
                        f"{net_accs[max_idx]:.1f}",
                        xy=(x[max_idx], net_accs[max_idx]),
                    ...
                    ax.annotate(f"T: {sorted_thresholds[max_idx]}", xy=(x[max_idx], net_accs[max_idx]), xytext=(0, 10),
                                textcoords='offset points', ha='center', fontweight='bold', color='purple', fontsize=10)
