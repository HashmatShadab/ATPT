...
plot_ensembling_comparison([clean_accs_single, clean_accs_vanilla, clean_accs_weighted],
                           f"Ensembling Comparison: Clean Setting ({model})", filename=f"I_zs_aug_ensemble_{model}_clean_comparison.png")

# Adversarial (Image-Text) Setting
plot_ensembling_comparison([adv_accs_single, adv_accs_vanilla, adv_accs_weighted],
                           f"Ensembling Comparison: Adversarial (Image-Text) ({model})", filename=f"I_zs_aug_ensemble_{model}_adv_img_text_comparison.png")
...
conditions = ['Clean', 'Adv (Img-Txt)',]
comp_methods = ['Single', 'Vanilla', 'Weighted']
comp_colors = ["#4A90E2", "#50E3C2", "#F5A623"]

avg_data = {
    'Single': [np.mean(clean_accs_single), np.mean(adv_accs_single), ],
    'Vanilla': [np.mean(clean_accs_vanilla), np.mean(adv_accs_vanilla), ],
    'Weighted': [np.mean(clean_accs_weighted), np.mean(adv_accs_weighted), ]
}
...
sns.despine()
plt.tight_layout()
plt.savefig(f"I_zs_aug_ensemble_{model}_overall_performance_summary.png", bbox_inches='tight')
plt.show()
