...
class CLIP(nn.Module):
    ...
    def encode_image(self, image, normalize: bool = False, get_all_layers=False, get_block_features=True):
        if not get_all_layers:
            if get_block_features:
                features, features_after_attention, features_after_mlp = self.visual(image, get_block_features=True)
                if normalize:
    ...
    def encode_text(self, text, normalize: bool = False):
        ...
        x = x + self.positional_embedding[:40].to(cast_dtype)
    ...
    def encode_text_embeddings(self, prompts, text, normalize: bool = False, context_length=40):
        cast_dtype = self.transformer.get_cast_dtype()

        x = prompts + self.positional_embedding[:context_length].to(cast_dtype)
        x = self.transformer(x, attn_mask=self.attn_mask)
        x = self.ln_final(x)  # [batch_size, n_ctx, transformer.width]
        x = text_global_pool(x, text, self.text_pool_type)
        if self.text_projection is not None:
