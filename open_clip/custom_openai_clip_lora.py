...
def create_lora_config(target_modules=None):
...
def get_lora_target_layers(
    ...
    module_name: str = "attn"
) -> list[str]:
    """
    Constructs target module names for selected transformer layers.

    Args:
        base (str): Base module prefix, e.g., "visual.transformer" or "transformer".
        total_layers (int): Total number of transformer layers (e.g., 12).
        selected_layers (list[int]): Indices of the layers to apply LoRA.
        module_name (str): Submodule to apply LoRA to, e.g., "attn", "q_proj", "v_proj".

    Returns:
        List[str]: List of target module names for use in LoraConfig.
    """
    if not all(0 <= i < total_layers for i in selected_layers):
        raise ValueError("Selected layer indices must be within total_layers range.")

    return [f"resblocks.{i}.{module_name}" for i in selected_layers]


def apply_peft(module, lora_config):
    peft_module = get_peft_model(module, lora_config)
    peft_module.print_trainable_parameters()
    return peft_module

def inspect_keys(original_state_dict, new_state_dict, prefix=""):
    original_keys = [k.replace(prefix, "") for k in original_state_dict]
    new_keys = [k.replace("base_model.model.", "").replace(prefix, "") for k in new_state_dict]

    exists = [k for k in new_keys if k in original_keys]
    not_exists = [k for k in new_keys if k not in original_keys]

    print(f"  - {len(exists)} keys already exist in the original model")
    print(f"  - {len(not_exists)} keys do not exist in the original model")
    for k in not_exists:
        print(f"    - {k}")
    return exists, not_exists

def save_lora_weights(peft_module):
    """Save current LoRA weights for later reset"""
    return {name: param.data.clone() for name, param in peft_module.named_parameters() if 'lora_' in name}

def reset_lora_weights(peft_module, saved_weights):
    """Reset LoRA weights to saved state"""
    for name, param in peft_module.named_parameters():
        if name in saved_weights:
            param.data.copy_(saved_weights[name])
def freeze_base_model_parameters(model):
    """Freeze all base model parameters, only keep LoRA trainable."""
    for name, param in model.named_parameters():
        # Only keep parameters with 'lora_' in their name trainable
...
class ClipTestTimeTuning(nn.Module):
    def __init__(self, device, classnames, batch_size, criterion='cosine', arch="ViT-L/14",
        ...
        model, _, _, tokenizer = get_open_clip(arch, device=device, cache_dir=DOWNLOAD_ROOT)

        number_of_text_blocks = len(model.transformer.resblocks)
        number_of_vision_blocks = len(model.visual.transformer.resblocks)
        print("Original model parameter counts:")
        print(f"  - Full model: {len(model.state_dict().keys())}")
        print(f"  - Visual encoder: {len(model.visual.state_dict().keys())}")

        vision_target_modules = get_lora_target_layers(
            base="visual.transformer",
            total_layers=number_of_vision_blocks,
        ...
        # Apply PEFT on image encoder
        print("\nApplying LoRA on image encoder...")
        peft_visual = apply_peft(model.visual, create_lora_config(vision_target_modules))

        # Replace original encoders with PEFT-enabled ones
        model.visual = peft_visual

        print("\nFinal model parameter counts after LoRA:")
        print(f"  - Full model: {len(model.state_dict().keys())}")
        print(f"  - Visual encoder: {len(model.visual.state_dict().keys())}")
        ...
        # prompt tuning
        self.prompt_learner = PromptLearner(model, classnames, batch_size, n_ctx, ctx_init, ctx_position, learned_cls, tokenizer, model_name=arch)
        self.criterion = criterion

        self.normalize = ImageNormalizer(mu, std).cuda(device)
        # Save only LoRA parameters
        self.lora_initial_state = {
            k: v.clone().detach() for k, v in self.model.visual.state_dict().items() if "lora" in k
        }
    ...
    def check_lora_diff(self, rtol=1e-5, atol=1e-8):
        """
        Compares current LoRA parameters with their initial state.
        Reports parameter name and max absolute difference.
        """
        current_state = self.model.visual.state_dict()
        diffs_found = False

        for name, initial_tensor in self.lora_initial_state.items():
            if name not in current_state:
                print(f"[Missing] {name} not in current state dict")
                continue

            current_tensor = current_state[name]
            if not torch.allclose(current_tensor, initial_tensor, rtol=rtol, atol=atol):
                max_diff = (current_tensor - initial_tensor).abs().max().item()
                print(f"[DIFF] {name}: max abs diff = {max_diff:.6e}")
                diffs_found = True

        if not diffs_found:
            print("✅ All LoRA parameters match the initial state.")

    def reset_lora(self):
        state_dict = self.model.visual.state_dict()
        for k, v in self.lora_initial_state.items():
            if k in state_dict:
                state_dict[k].copy_(v)
            else:
                print(f"Warning: {k} not found in current model state.")
        self.check_lora_diff()
