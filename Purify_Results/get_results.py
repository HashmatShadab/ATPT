...
FINAL_RE = re.compile(r"(INFO - Final Clean orig accuracy:.*|INFO - Final Adv orig accuracy:.*|INFO - Diff_ratio_clean:.*)", re.I)

def last_final_line(path: Path) -> Optional[str]:
    """Return the last line containing accuracy results (or None).

    Matches patterns like:
    - 'INFO - Final Clean orig accuracy: ... | Clean purify accuracy: ...'
    - 'INFO - Final Adv orig accuracy: ... | Adv purify accuracy: ...'
    - 'INFO - Diff_ratio_clean: ... | Diff_ratio_adv: ...'
    """
    ...
    try:
        with path.open(encoding="utf-8", errors="ignore") as fh:
            ...
            def get_result_order(result):
                if "INFO - Final Clean orig accuracy:" in result:
                    return 0
                elif "INFO - Final Adv orig accuracy:" in result:
                    return 1
                elif "INFO - Diff_ratio_clean:" in result:
                    return 2
                else:
                    return 3
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Extract accuracy metrics from log files (INFO - Final Clean/Adv orig accuracy and Diff_ratio)."
    )
